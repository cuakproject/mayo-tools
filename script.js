...
// GANTI INI DENGAN URL BACKEND LO
const ROBLOX_PROXY_URL = 'https://roblox-backend-rust.vercel.app'; // Ganti ke URL backend lo (Vercel/Railway)
...
function showTemplatesGrid() {
    ...
    if (btn) btn.innerHTML = '<i class="fas fa-eye-slash"></i><span>Sembunyikan</span>';
...
function hideTemplatesGrid() {
    ...
    if (btn) btn.innerHTML = '<i class="fas fa-eye"></i><span>Tampilkan</span>';
...
function initializeBackupFormatter() {
    ...
    input.addEventListener('input', function () {
        clearTimeout(timer);
        timer = setTimeout(() => {
            if (this.value.trim()) generateBackupFormat();
        }, 300);
    });
...
function extractBackupCodes(text) {
    ...
    const isCode = s => {
        ...
        return t.length === 9 && /^[a-z0-9]+$/.test(t) && !/^\d+$/.test(t) && !INV_WORD_BLOCK.has(t);
    };
    const addCode = s => {
        const t = s.toLowerCase().trim().replace(/[`'"]/g, '');
    ...
    for (const l of lines) {
        if (!lp.test(l)) continue;
        const v = l.replace(lp, '').replace(/[-•\s`'"]/g, '').trim();
        if (addCode(v) && codes.length >= 5) return codes;
    }
    if (codes.length >= 5) return codes;
    for (const l of lines) {
        const m = l.match(/^\s*\d+[.)]\s*([a-z0-9]{9})\s*$/i);
...
async function pasteFromClipboard() {
    try {
        ...
        showToast('❌ Gagal baca clipboard');
    }
}
...
function invFormatRobux(raw) {
    ...
    if (isPrem) {
        ...
        const b = nums.filter(n => n >= 100);
        if (b.length) { const c = P.reduce((a, v) => Math.abs(a - b[0]) <= Math.abs(v - b[0]) ? a : v); return c + 'R + Premium'; }
...
function invExtractUsername(text) {
    ...
    const tryX = m1 => {
        if (!m1) return '';
        return m1.trim().replace(/^[🫧🌸✨👤🔑🛡\s@*_]+|[🫧🌸✨👤🔑🛡\s@*_]+$/g, '').split(/\s+/)[0].trim();
    };
    const em = text.match(ep);
    if (em) { const v = tryX(em[1]); if (v && v.length >= 2) return v; }
    let m; lp.lastIndex = 0;
    while ((m = lp.exec(text)) !== null) { const v = tryX(m[1]); if (v && v.length >= 2) return v; }
    return '';
}
...
function invExtractPassword(text) {
    ...
    const em = text.match(ep);
    if (em) { const v = tryX(em[1]); if (v && v.length >= 1) return v; }
    let m; lp.lastIndex = 0;
    while ((m = lp.exec(text)) !== null) { const v = tryX(m[1]); if (v && v.length >= 1) return v; }
    return '';
}
...
function invExtractRobux(text) {
    ...
    const st = sl.join('\n');
    const isPrem = /prem(?:ium)?/i.test(sl.filter(l =>
    ...
    while ((m = op.exec(norm)) !== null) {
        ...
        if (isPrem) {
            const P = [450, 1000, 2200];
            for (const n of nums) if (P.includes(n)) return n + 'prem';
            if (nums.includes(2000)) return '2200prem';
            if (nums.length) { const c = P.reduce((a, b) => Math.abs(a - nums[0]) <= Math.abs(b - nums[0]) ? a : b); return c + 'prem'; }
            return '1000prem';
        } else {
    ...
    if (isPrem) {
        const P = [450, 1000, 2200];
        for (const n of all) if (P.includes(n)) return n + 'prem';
        if (all.includes(2000)) return '2200prem';
        if (all.length) { const c = P.reduce((a, b) => Math.abs(a - all[0]) <= Math.abs(b - all[0]) ? a : b); return c + 'prem'; }
        return '1000prem';
    }
...
function invExtractCodes(text) {
    ...
    const addCode = tok => {
        ...
        if (t.length === 9 && /^[a-z0-9]+$/.test(t) && !/^\d+$/.test(t) && !INV_WORD_BLOCK.has(t) && !codes.includes(t)) {
            codes.push(t);
            return codes.length >= 5;
        }
        return false;
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (!blp.test(line)) continue;
        const chunk = [line];
        for (let j = i + 1; j <= i + 2 && j < lines.length; j++) {
            const nx = lines[j].trim();
            if (blp.test(nx) && !/^[a-z0-9]{9}$/i.test(nx.replace(/\W/g, ''))) break;
            if (/(?:username|password|order|bukti|payment|foto)/i.test(nx) && !/^[a-z0-9]{9}$/i.test(nx.replace(/\W/g, ''))) break;
            chunk.push(nx);
        }
        for (const tok of chunk.join(' ').replace(blp, ' ').split(/[\s,;|`'\-]+/)) {
            if (addCode(tok) && codes.length >= 5) return codes;
        }
    }
    ...
    if (codes.length >= 5) return codes.slice(0, 5);
    for (const l of lines) {
        const m = l.match(/^\s*[-•·*]\s*([a-z0-9]{9})\s*$/i);
        if (m && addCode(m[1]) && codes.length >= 5) return codes;
    }
    if (codes.length >= 5) return codes.slice(0, 5);
    const fl = lines.filter(l =>
...
async function inv4DoPaste() {
    try {
        ...
        showToast('❌ Gagal baca clipboard. Izinkan akses dulu.');
    }
}

// ==================== INV4: RAW PASTE / INPUT ====================
function inv4OnRawPaste(event) {
    clearTimeout(inv4RawTimer);
    const pasted = event && event.clipboardData && event.clipboardData.getData('text');
    const delay = (pasted && pasted.trim().length > 15) ? 80 : 200;
...
function inv4Clear() {
    ...
    const rawPaste = document.getElementById('inv4RawPaste');
    ...
    if (raw) { raw.value = ''; raw.style.display = 'none'; }
    ...
    ['inv4CardRobux', 'inv4CardUser', 'inv4CardPass', 'inv4CardBackup'].forEach(id => {
        ...
        if (el) el.classList.remove('state-ok', 'state-err');
    });
...
function inv4Parse(raw) {
    ...
    if (!username && !password && !robuxFormatted && !codes.length) {
        showToast('⚠️ Tidak ada data yang bisa dibaca, coba paste ulang');
    }
...
function inv4UpdateStatuses() {
    ...
    const cR = document.getElementById('inv4CardRobux');
    if (cR) { cR.classList.toggle('state-ok', !!r); cR.classList.remove('state-err'); }

    const u = document.getElementById('inv4User').value.trim();
    if (!u) {
        inv4SetStatus('inv4StatusUser', 'warn', '—');
        const cU = document.getElementById('inv4CardUser');
        if (cU) { cU.classList.remove('state-ok', 'state-err'); }
    }

    const p = document.getElementById('inv4Pass').value.trim();
    inv4SetStatus('inv4StatusPass', p ? 'ok' : 'warn', p ? '✓' : '—');
    const cP = document.getElementById('inv4CardPass');
    if (cP) { cP.classList.toggle('state-ok', !!p); cP.classList.remove('state-err'); }

    const n = inv4State.codes.length;
    inv4SetStatus('inv4StatusBackup', n >= 5 ? 'ok' : 'warn', n >= 5 ? '5/5 ✓' : n + '/5');
    const cB = document.getElementById('inv4CardBackup');
    if (cB) { cB.classList.toggle('state-ok', n >= 5); cB.classList.remove('state-err'); }
}
...
function inv4RenderChips(codes) {
    ...
    codes.forEach(c => {
        ...
        chip.className = 'inv4-chip';
        chip.textContent = c;
        wrap.appendChild(chip);
    });
    for (let i = codes.length; i < 5; i++) {
        const chip = document.createElement('span');
        chip.className = 'inv4-chip miss';
        chip.textContent = 'kode ' + (i + 1);
        wrap.appendChild(chip);
    }
}
...
function inv4CopySingleField(field) {
    ...
    switch (field) {
        case 'user':
            text = document.getElementById('inv4User').value.trim();
            label = 'Username';
            break;
        case 'pass':
            text = document.getElementById('inv4Pass').value.trim();
            label = 'Password';
            break;
        case 'robux':
            text = document.getElementById('inv4Robux').value.trim();
            label = 'Jumlah Robux';
            break;
        case 'backup':
            text = inv4State.codes.join(', ');
            label = 'Backup Code';
            break;
    }
    ...
    const doCopy = () => {
        ...
        const cardId = field === 'user' ? 'inv4CardUser' :
            field === 'pass' ? 'inv4CardPass' :
                field === 'robux' ? 'inv4CardRobux' : 'inv4CardBackup';
        const card = document.getElementById(cardId);
        if (card) {
            card.classList.add('copied-flash');
            setTimeout(() => card.classList.remove('copied-flash'), 800);
        }
...
function inv4Copy() {
    ...
    if (!robux && !user && !pass) {
        showToast('❌ Isi dulu data customernya!');
        return;
    }
    ...
    const doCopy = () => {
        ...
        document.execCommand('copy');
        document.body.removeChild(ta);
        const msg = missing.length ? `⚠️ Dicopy (kurang: ${missing.join(', ')})` : '✅ Invoice dicopy! Paste ke Telegram 🎉';
        showToast(msg);
    };
...
function inv4ShowAvatarLoading(msg) {
    ['inv4AvatarEmpty', 'inv4AvatarFound', 'inv4AvatarErr', 'inv4UserCheck', 'inv4UserX'].forEach(id => {
        const el = document.getElementById(id); if (el) el.style.display = 'none';
    });
    const loading = document.getElementById('inv4AvatarLoading');
...
function inv4ShowAvatarErr(msg) {
    ...
    ['inv4AvatarErr', 'inv4UserX'].forEach(id => {
...
function renderResellerUI() {
    ...
    const si = document.getElementById('rsSearchInput');
    const cb = document.getElementById('rsClearSearch');
...
function filterResellers(q) {
    document.querySelectorAll('.rs-reseller-card').forEach(card => {
        const r = resellerData[parseInt(card.dataset.index)];
        if (!q) { card.style.display = 'flex'; return; }
        card.style.display = (r.displayName.toLowerCase().includes(q) || r.username.toLowerCase().includes(q)) ? 'flex' : 'none';
    });
}
...
function updateRsResult() {
    ...
    if (cmd) {
        ...
        let h = '—';
        if (!rsState.reseller && rsState.command === 'proses' && !rsState.nominal) h = 'Pilih nominal & reseller';
        else if (rsState.command === 'proses' && !rsState.nominal) h = 'Pilih nominal dulu';
        else if (!rsState.reseller) h = 'Pilih reseller dulu';
...
function copyRsResult() {
    ...
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(cmd).then(() => showToast(`✅ Command dicopy: ${cmd}`)).catch(dc);
    } else { dc(); }
}
...
window.inv4CopySingleField = inv4CopySingleField;
